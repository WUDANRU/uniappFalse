<template>
	<view class="bg-white" @click.prevent="openDetail">
			...
			v-for="(order,orderIndex) in item.order_items" :key="orderIndex">
				...
				<view class="flex-1">
					<view class="d-flex a-center">
						<text class="font-3 text-dark">{{order.title}}</text>
						<text class="font-3 text-light-muted ml-auto"
						>￥{{order.pprice}}</text>
					</view>
					<view class="d-flex a-center">
						<text class="font-1 text-light-muted">{{order.attrs}}</text>
						<text class="font-1 text-light-muted ml-auto"
						>x{{order.num}}</text>
					</view>
				</view>
			</view>
		...
		<view class="d-flex j-end a-center px-2 pb-2">
			<view class="rounded border border-light-secondary mr-2 py-1 px-2 text-secondary"
			...
			hover-class="bg-light-secondary" @click.stop="openLogistics">
				查看物流
			</view> 
